...
このプロジェクトは `vite-plugin-pwa` を使用してPWA機能を提供しています。
...
- `dist/client/manifest.webmanifest`: Web App Manifest
- `dist/client/sw.js`: Service Worker
- `dist/client/registerSW.js`: Service Worker登録スクリプト

### 開発時の確認

開発サーバー起動後、ブラウザの開発者ツールで以下を確認できます：

1. **Application タブ → Service Workers**: Service Workerが登録されているか確認
2. **Application タブ → Manifest**: PWA設定が正しく読み込まれているか確認
3. **Network タブ**: オフライン時にキャッシュが機能しているか確認

### アイコンのカスタマイズ

現在は `/vite.svg` をアイコンとして使用しています。より適切なアイコンを設定する場合：

1. `public/` ディレクトリに適切なサイズのアイコンを配置（例: `icon-192.png`, `icon-512.png`）
2. `vite.config.ts` の `manifest.icons` を更新
...
- PWAファイル（manifest.webmanifest、sw.js）も自動的にデプロイされます
