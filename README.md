...
|F-07|データ入力（フロントエンド） |画面上のモーダルフォームから月・種別・カテゴリ・金額を入力し**追加のみ**可能（編集・削除はスプシで直接行う）。選択可能な月は開始月から先々月までで、既にデータが存在する月は選択不可|
...
2. モーダルで月を選択（ドロップダウン）
3. 各カテゴリの金額を入力（支出・収入それぞれ）
4. 「まとめて追加」ボタンをクリック
5. 追加後、グラフは自動的に更新され、モーダルが閉じます
